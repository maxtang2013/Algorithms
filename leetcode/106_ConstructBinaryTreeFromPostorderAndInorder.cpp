...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        return buildTree(postorder, 0, inorder.size(), inorder, 0, inorder.size());
    }
    
private:
    TreeNode* buildTree(const vector<int>& postorder, int pfrom, int pto, const vector<int>& inorder, int from, int to) {
        if (to == from) return NULL;
        if (to == from + 1) {
            return new TreeNode(inorder[from]);
        }
        
        int val = postorder[pto - 1];
        int len = 0;
        ...
        node->left = buildTree(postorder, pfrom, pfrom + len, inorder, from, from + len);
        node->right = buildTree(postorder, pfrom + len, pto-1, inorder, from + len + 1, to);
        return node;
    }
};
