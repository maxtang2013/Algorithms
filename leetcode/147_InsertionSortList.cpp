...
public:
    ListNode* insertionSortList(ListNode* head) {
        ListNode dummy(0);
        ListNode* curr = head, *insertPoint;
        
        while (curr != NULL) {
            insertPoint = &dummy;
            while (insertPoint->next != NULL && curr->val > insertPoint->next->val) {
                insertPoint = insertPoint->next;
            }
            ListNode* next = insertPoint->next;
            insertPoint->next = curr;
            curr = curr->next;
            insertPoint->next->next = next;
        }
        return dummy.next;
    }
