...
public:
    int calculateMinimumHP(vector<vector<int>>& dungeon) {
        int low = 1, hi = INT_MAX;
        while (low < hi) {
            int mid = low + (hi - low) / 2;
            if (canReachBottomRight(mid, dungeon))
                hi = mid;
            else low = mid + 1;
        }
        return hi;
...
private:
    bool canReachBottomRight(int hp, vector<vector<int>>& dungeon) {
        int m = dungeon.size();
        int n = dungeon[0].size();
        
        if (hp + (long long)dungeon[0][0] <= 0) return false;
        
        vector<vector<long long>> maxSum(m);
        for (int i = 0; i < m; ++i) {
            maxSum[i].resize(n);
        }
        
        maxSum[0][0] = hp + dungeon[0][0];
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                if (i > 0 && maxSum[i-1][j] > 0) {
                    maxSum[i][j] = max(maxSum[i][j], maxSum[i-1][j] + dungeon[i][j]);
                }
                
                if (j > 0 && maxSum[i][j-1] > 0) {
                    maxSum[i][j] = max(maxSum[i][j], maxSum[i][j-1] + dungeon[i][j]);
                }
            }
        }
        return maxSum[m-1][n-1] > 0;
    }
};
...
{
    Solution sln;
    vector<vector<int>> m = {{1,-3,3},{0,-2,0},{-3,-3,-3}};
    cout <<sln.calculateMinimumHP(m) << endl;
}
