...
public:
    vector<int> largestDivisibleSubset(vector<int>& nums) {
        int n = (int) nums.size();
        vector<int> longestLengthEndAt(n, 1);
        vector<int> ans;
        vector<int> prev(n, -1);
        int longest = 0;
        int longestTail = -1;
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < i; ++j) {
                if (nums[i] % nums[j] == 0 && longestLengthEndAt[j] + 1 > longestLengthEndAt[i]) {
                    prev[i] = j;
                    longestLengthEndAt[i] = longestLengthEndAt[j] + 1;
                }
            }
            if (longest < longestLengthEndAt[i]) {
                longest = longestLengthEndAt[i];
                longestTail = i;
            }
        }
        
        for (int i = longestTail; i != -1; i = prev[i])
        {
